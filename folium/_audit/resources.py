"""
CDN Resource Consistency Audit for Folium.
        ...
        for url in doc_urls:
            if url not in manifest_urls:
                result.add_warning(
                    ...
                    "CDN URL in docs not found in manifest",
        ...
        for node in ast.walk(tree):
            ...
            if isinstance(node.value, ast.Constant) and isinstance(node.value.value, str):
                ...
                if CDN_PATTERN.match(url):
                    result.add_warning(
                        "inline_cdn_url",
                        "CDN URL found outside default_js/default_css",
                        location=str(rel),
                        detail=url[:120],
                    )
...
def run_audit(strict: bool = False) -> AuditResult:
    """Run the full CDN resource consistency audit."""
...
def collect_resource_summary() -> dict[str, Any]:
    """Collect a high-level summary of the resource manifest."""
    manifest = load_manifest()
    ...
    return {
        ...
        "map_defaults": {
            ...
            "js_urls": [
                r["url"] for r in manifest.get("map_defaults", {}).get("js", [])
            ],
            "css_urls": [
                r["url"] for r in manifest.get("map_defaults", {}).get("css", [])
            ],
        },
        "features": features,
...
def collect_plugin_resources() -> list[dict[str, Any]]:
    """Collect detailed resource info for all plugins."""
    ...
    for name in plugins.__all__:
        ...
        if cls is not None and isinstance(cls, type):
            ...
            result.append({
                ...
                "js_urls": [u for _, u in default_js],
                "css_urls": [u for _, u in default_css],
            })
    return result
